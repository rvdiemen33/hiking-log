...
/// <summary>Tier 0 tests for GET /hikelogs/{id}.</summary>
[Collection(nameof(HikingLogTier0Collection))]
public class GetHikeLogTests(HikingTestWebApplicationFactory factory) : IntegrationTest(factory)
{
    /// <summary>GET /hikelogs/{id} returns 200 OK when the hike log exists.</summary>
    [Fact]
    public async Task GetHikeLog_WhenExists_Returns200()
    {
    ...
    /// <summary>GET /hikelogs/{id} returns 404 Not Found when the hike log does not exist.</summary>
    [Fact]
    public async Task GetHikeLog_WhenNotFound_Returns404()
    {
        var client = CreateClient();
        var response = await client.GetAsync("/hikelogs/99999");
